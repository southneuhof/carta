...
**In:** `packages/loom/src/index.ts`, `packages/loom/src/contracts/{forms,tables,details,display,labels,schema}.ts`, `contracts/index.ts`, `forms/{defineForm,compileForm,behavior,props,index}.ts`, `tables/{defineTable,index}.ts`, `details/{defineDetail,index}.ts`, `labels/{resolveLabel,index}.ts`, `schemas/{compileSchema,index}.ts`, `renderers/{registry,displayContracts,index}.ts`, focused tests and included `__type-tests__`, `packages/loom/tsconfig.json`, `plans/resource-system-overhaul-inventory.md`.
...
Keep all eight plans on the current integration branch, `resource_system_overhaul`. Do not ship an intermediate state. Do not commit or push unless asked.
