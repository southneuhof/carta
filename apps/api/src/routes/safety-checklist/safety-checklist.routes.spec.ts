...
import { safetyChecklists } from './safety-checklist.entity'
...
function id(prefix: string) {
  return `safety-checklist-test-${prefix}-${crypto.randomUUID()}`
}
...
async function makeSession(permissionCodes: readonly string[]): Promise<Fixture> {
  ...
  await db.insert(users).values({ id: userId, name: 'Safety Checklist Test User', email })
  ...
  await db.insert(authorizationModules).values({ id: moduleId, code: id('module-code'), name: 'Safety Checklist Test', realm: 'system' })
  await db.insert(roles).values({ id: roleId, roleCode: id('role-code'), name: 'Safety Checklist Test Role', realm: 'system' })
...
describe('Safety Checklist routes', () => {
  afterEach(async () => {
    const db = getDb()
    while (fixtures.length) {
      const fixture = fixtures.pop()!
      await db.delete(safetyChecklists).where(or(
        eq(safetyChecklists.createdByUserId, fixture.userId),
        eq(safetyChecklists.updatedByUserId, fixture.userId),
      ))
  ...
  it('requires authentication', async () => {
    expect((await app.request('/safety-checklist/list')).status).toBe(401)

    const limited = await makeSession([])
    expect((await app.request('/safety-checklist/list', { headers: { Cookie: limited.cookie } })).status).toBe(403)
    expect((await app.request('/safety-checklist/create', {
      method: 'POST',
  ...
  it('supports validated audited CRUD and nullable unique codes', async () => {
    ...
    const invalidName = await app.request('/safety-checklist/create', {
      method: 'POST',
      headers,
    ...
    const invalidActive = await app.request('/safety-checklist/create', {
      method: 'POST',
      headers,
    ...
    const createdResponse = await app.request('/safety-checklist/create', {
      method: 'POST',
      headers,
      body: JSON.stringify({ name: '  Safety Copy  ', description: 'Description', code: '  SC-COPY  ' }),
    })
    ...
    const blankCodeResponse = await app.request('/safety-checklist/create', {
      method: 'POST',
      headers,
    ...
    const duplicateCode = await app.request('/safety-checklist/create', {
      method: 'POST',
      headers,
      body: JSON.stringify({ name: 'Duplicate Code', code: 'SC-COPY' }),
    })
    expect(duplicateCode.status).toBe(400)

    const listResponse = await app.request('/safety-checklist/list?page=1&limit=20', { headers: { Cookie: fixture.cookie } })
    expect(listResponse.status).toBe(200)
    const listed = await listResponse.json() as { data: Array<Record<string, unknown>> }
    expect(listed.data.some((row) => row.id === created.id && row.name === 'Safety Copy')).toBe(true)

    const detailResponse = await app.request(`/safety-checklist/detail/${String(created.id)}`, { headers: { Cookie: fixture.cookie } })
    expect(detailResponse.status).toBe(200)
    expect((await detailResponse.json() as { data: Record<string, unknown> }).data).toMatchObject({ id: created.id, name: 'Safety Copy', code: 'SC-COPY', description: 'Description' })

    const updateResponse = await app.request(`/safety-checklist/update/${String(created.id)}`, {
      method: 'PATCH',
      headers,
      body: JSON.stringify({ name: '  Updated Safety Checklist ', description: null, active: false, code: ' UPDATED ' }),
    })
    ...
    const partialUpdateResponse = await app.request(`/safety-checklist/update/${String(created.id)}`, {
      method: 'PATCH',
      headers,
    ...
    const deleteResponse = await app.request(`/safety-checklist/delete/${String(created.id)}`, { method: 'DELETE', headers: { Cookie: fixture.cookie } })
    expect(deleteResponse.status).toBe(200)
    expect((await app.request(`/safety-checklist/detail/${String(created.id)}`, { headers: { Cookie: fixture.cookie } })).status).toBe(404)
  })
})
