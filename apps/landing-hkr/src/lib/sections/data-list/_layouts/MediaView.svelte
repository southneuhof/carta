  ...
  <div class="flex flex-col gap-sm">
    ...
    <div class="flex flex-row items-center gap-base flex-wrap text-sm">
      ...
      {#if content.attachment && !content.status}
        <a href={`/media-preview?url=${encodeURIComponent(assetUrl(content.attachment))}&title=${encodeURIComponent(content.title ?? '')}&description=${encodeURIComponent(content.description ?? '')}`}><span class="underline">Preview</span> <i class="ri-eye-line"></i></a>
        <a href={assetUrl(content.attachment)} target="_blank" rel="noopener noreferrer"><span class="underline">Download</span> <i class="ri-download-line"></i></a>
      {/if}
