  ...
  ? TRoute['update'] extends infer TUpdate
    ? ':id' extends keyof TUpdate
      ? TUpdate[':id'] extends infer TNode
        ? '$patch' extends keyof TNode ? TNode['$patch'] : never
        : never
      : never
    : never
  : never
  ...
  ? TRoute['delete'] extends infer TDelete
    ? ':id' extends keyof TDelete
      ? TDelete[':id'] extends infer TNode
        ? '$delete' extends keyof TNode ? TNode['$delete'] : never
        : never
      : never
    : never
  : never
...
export type HonoRecordOf<TRoute> = [ListEndpoint<TRoute>] extends [never]
  ...
  : ListRecordOf<HonoResponseOf<ListEndpoint<TRoute>, 200>> extends object
    ...
    : Record<string, unknown>
export type HonoQueryOf<TRoute> = [ListEndpoint<TRoute>] extends [never]
  ? Record<string, never>
  : AdapterQuery<QueryOfEndpoint<ListEndpoint<TRoute>> & object>
...
export type AppResourceContract<TRoute> = WebResourceSchema<
  HonoRecordOf<TRoute>,
  HonoQueryOf<TRoute>,
  HonoCreateOf<TRoute>,
  HonoUpdateOf<TRoute>,
...
export type HonoResourceActions<TRoute> =
  ([ListEndpoint<TRoute>] extends [never] ? {} : {
