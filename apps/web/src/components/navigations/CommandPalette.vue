...
function routeCode(to: unknown, fallback: string) {
  return (
    router
      .resolve(to as never)
      .path.split('/')
...
const filteredRoutes = computed(() => {
  ...
  return routes.value.filter((entry) => [entry.title, entry.moduleTitle, ...entry.aliases].join(' ').toLocaleLowerCase().includes(value))
})
