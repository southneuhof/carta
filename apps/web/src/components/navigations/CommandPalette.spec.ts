...
describe('CommandPalette', () => {
  it('opens from the trigger and filters route and module titles immediately', async () => {
    ...
    await view.get('[data-command-palette-input]').setValue('Pengaturan')

    expect(view.findAll('[role="option"]').map((option) => option.attributes('aria-label'))).toEqual(['Users — Pengaturan', 'Roles — Pengaturan', 'Permissions — Pengaturan'])
  })
  ...
  it('selects a route with the keyboard and closes the palette', async () => {
    ...
    expect(mocks.push).toHaveBeenCalledExactlyOnceWith({ name: 'to-do' })
