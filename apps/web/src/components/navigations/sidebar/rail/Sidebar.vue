      ...
      <section v-for="module in navigation" :key="module.name" class="mb-5">
        ...
        <template v-for="entry in module.routes" :key="entry.name">
          ...
          <RouterLink
            ...
            class="flex min-h-11 items-center gap-3 rounded-lg px-3 py-2 text-sm outline-none hover:bg-surface-variant focus-visible:ring-2 focus-visible:ring-primary"
            :class="entry.name === String(route.name) || module.name === activeModule && route.path.startsWith(router.resolve(entry.to as never).path) ? 'bg-primary-container text-on-primary-container' : ''"
          >
