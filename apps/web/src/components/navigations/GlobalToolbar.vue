...
<template>
  <header class="sticky top-0 z-40 grid h-16 grid-cols-[minmax(0,1fr)_auto] items-center border-b border-outline-variant bg-surface-container px-4 text-on-surface lg:static lg:h-14 lg:px-6">
      ...
      <nav aria-label="Breadcrumb" class="min-w-0 overflow-hidden">
        <ol class="flex min-w-0 items-center gap-2 whitespace-nowrap text-sm">
          <li v-for="(item, index) in breadcrumbs" :key="`${item.label}-${index}`" class="flex min-w-0 items-center gap-2">
            ...
            <span v-else class="truncate font-medium" :aria-current="index === breadcrumbs.length - 1 ? 'page' : undefined">{{ item.label }}</span>
          </li>
        </ol>
      </nav>
    </div>
