import type { FrameworkFieldDefaultsInput } from '@southneuhof/loom'
 ...
 * Validation belongs to resource schemas and form initial values to these
 * field defaults when the action does not provide a value.
 */
export const appFieldDefaults = {
  fields: {
    ...
    active: {
      ...
      form: { renderer: 'switch', props: { required: true }, initialValue: () => true },
    },
    statusCode: {
      label: 'Status',
    ...
    file: { label: 'File' },
    updatedBy: { label: 'Diperbarui Oleh', display: { read: (record) => record.relUpdatedBy } },
    createdBy: { label: 'Dibuat Oleh', display: { read: (record) => record.relCreatedBy } },
