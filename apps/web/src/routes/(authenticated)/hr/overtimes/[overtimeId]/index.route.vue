...
import { DetailView, ListView, standardControls, type ViewControl } from '@southneuhof/is-vue-framework'
import {
  loadOvertime,
  overtimes,
  submitOvertime,
...
definePage({ name: 'overtime-detail', meta: { title: 'Detail Lembur', moduleName: 'hr' } })
...
const stepsTable = computed(() => verificationSteps.table({ searchParameters: { overtime_id: overtimeId.value } }))

async function refresh() {
  if (!overtimeId.value) return
  record.value = await loadOvertime(overtimeId.value)
  const loaded = await verificationSteps.table().load!({ query: {}, searchParameters: { overtime_id: overtimeId.value } })
...
const controls = computed<ViewControl[]>(() => {
  const workflow: ViewControl[] = []
  ...
  return [
    ...standardControls({ resource: overtimes, surface: 'detail', id: overtimeId.value, record: record.value }),
    ...workflow,
  ]
})

const detail = computed(() => ({ ...overtimes.detail({ id: overtimeId.value }), data: record.value }))

function onBack() {
  void router.push(overtimes.routes.list!)
}
</script>

<template>
  <div>
    <DetailView title="Detail Lembur" :detail="detail" :controls="controls" />
