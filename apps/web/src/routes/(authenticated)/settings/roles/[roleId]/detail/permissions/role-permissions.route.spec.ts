...
describe('role permission editor', () => {
  it('shows same-realm permissions in one standard list and exposes accessible switches', async () => {
    const view = await mountScreen()
    expect(view.host.querySelectorAll('table')).toHaveLength(1)
    expect(view.host.textContent).toContain('Users')
    expect(view.host.textContent).toContain('Roles')
  ...
  it('rejects a cross-realm response as a contract error without switches', async () => {
    crossRealm = true
    const view = await mountScreen()
    expect(view.host.querySelector('[role="alert"]')?.textContent).toContain('Permission catalog realm mismatch.')
    expect(view.host.querySelectorAll('[data-permission]')).toHaveLength(0)
    view.unmount()
  })
