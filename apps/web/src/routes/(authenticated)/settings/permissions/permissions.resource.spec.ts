...
describe('permission resource', () => {
  it('is a read-only catalog with realm and module fields', () => {
    ...
    expect(resolveFields({ fields: permissionResource.detail({ id: 'permission-1' }).fields, surface: 'detail' }).map((field) => field.key)).toEqual(['permissionCode', 'name', 'module', 'realm', 'description', 'active'])
