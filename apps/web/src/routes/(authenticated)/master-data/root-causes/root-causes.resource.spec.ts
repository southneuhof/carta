...
describe('root causes resource', () => {
  it('uses the four business fields on every standard surface', () => {
    ...
    expect(fields(rootCauses.table().table.fields, 'table').map((field) => field.key)).toEqual(keys)
    expect(fields(rootCauses.detail({ id: '1' }).detail.fields, 'detail').map((field) => field.key)).toEqual(keys)
    const formFields = fields(rootCauses.form().fields, 'form')
