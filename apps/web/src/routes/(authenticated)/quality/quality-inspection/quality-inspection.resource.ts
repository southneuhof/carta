...
const fields = defineFields(qualityInspectionSchema, {
  ...
  divisionId: {
    label: 'Divisi',
    read: (record) => relationName(record, 'division', record.divisionName as string),
    form: {
      renderer: 'lookup',
      source: divisions,
      props: { pick: 'id', view: 'name', required: true },
      behavior: { props: () => ({ searchParameters: { permission: 'create-quality-inspection', active: true } }) },
    },
  },
  projectId: {
    label: 'Proyek',
    ...
    form: {
      ...
      behavior: {
        ...
        props: ({ draft }) => ({ searchParameters: { permission: 'create-quality-inspection', divisionId: draft.divisionId, active: true } }),
  ...
  qualityWorkCategoryId: {
    label: 'Kategori Pekerjaan',
    read: (record) => relationName(record, 'qualityWorkCategory'),
  ...
  workItemCategoryId: {
    ...
    read: (record) => relationName(record, 'workItemCategory', record.workItemCategoryName as string),
    form: {
      renderer: 'lookup',
      source: workItems,
      props: { pick: 'id', view: 'name', required: true },
      behavior: {
        disabled: ({ draft }) => !draft.projectId || !draft.qualityWorkCategoryId,
        props: ({ draft }) => ({ searchParameters: { projectId: draft.projectId, categoryId: draft.qualityWorkCategoryId, rootOnly: true, active: true } }),
        resetWhen: ({ draft }) => `${draft.projectId ?? ''}:${draft.qualityWorkCategoryId ?? ''}`,
      },
    },
  },
...
export const qualityInspection = defineResource(qualityInspectionSchema, {
  ...
  actions: {
    list: {
      run: qualityInspectionActions.list,
      fields: [fields.number, fields.projectId, fields.targetDate, fields.workItemCategoryId, fields.createdByName, fields.createdAt, fields.locationZone, fields.statusCode, fields.stepCode],
      permission: 'view-quality-inspection',
      route: { name: 'quality-quality-inspection' },
    },
    detail: {
      run: qualityInspectionActions.detail,
      fields: [
        fields.number,
        fields.divisionId,
        fields.projectId,
        fields.targetDate,
        fields.qualityWorkCategoryId,
        fields.workItemCategoryId,
        fields.locationZone,
        fields.createdByName,
        fields.scheduleId,
        fields.scheduleStartDate,
        fields.scheduleEndDate,
        fields.statusCode,
        fields.stepCode,
        fields.resultCode,
        fields.verificationDescription,
        fields.inspectionPointCode,
        fields.workMethod,
        fields.createdAt,
      ],
      permission: 'show-quality-inspection',
      route: { name: 'quality-quality-inspection-detail', params: (id) => ({ qualityInspectionId: String(id) }) },
    },
    create: {
      run: qualityInspectionActions.create,
      fields: [fields.divisionId, fields.projectId, fields.targetDate, fields.qualityWorkCategoryId, fields.workItemCategoryId, fields.locationZone, fields.selectedRows],
      permission: 'create-quality-inspection',
      route: { name: 'quality-quality-inspection-create' },
    },
    update: {
      run: qualityInspectionActions.update,
      fields: [fields.divisionId, fields.projectId, fields.targetDate, fields.qualityWorkCategoryId, fields.workItemCategoryId, fields.locationZone, fields.selectedRows],
      permission: 'update-quality-inspection',
      route: { name: 'quality-quality-inspection-edit', params: (id) => ({ qualityInspectionId: String(id) }) },
    },
