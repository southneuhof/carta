...
import { itpTypeOptions, qualityInspectionSchema, statusOptions, stepLabels } from './quality-inspection.schema'
...
const fields = defineFields(qualityInspectionSchema, {
  ...
  workItemCategoryId: { label: 'Jenis Pekerjaan', read: (record) => relationName(record, 'workItemCategory'), form: { renderer: 'lookup', source: workItems, props: { pick: 'id', view: 'name', required: true }, behavior: { disabled: ({ draft }) => !draft.projectId, props: ({ draft }) => ({ searchParameters: { projectId: draft.projectId, rootOnly: true, active: true } }), resetWhen: ({ draft }) => draft.projectId } } },
...
export const qualityInspection = defineResource(qualityInspectionSchema, {
  ...
  actions: {
    list: { run: qualityInspectionActions.list, fields: [fields.number, fields.projectId, fields.targetDate, fields.statusCode, fields.stepCode], permission: 'view-quality-inspection', route: { name: 'quality-quality-inspection' } },
    detail: { run: qualityInspectionActions.detail, fields: [fields.number, fields.divisionId, fields.projectId, fields.targetDate, fields.qualityWorkCategoryId, fields.workItemCategoryId, fields.locationZone, fields.statusCode, fields.stepCode, fields.createdAt], permission: 'show-quality-inspection', route: { name: 'quality-quality-inspection-detail', params: (id) => ({ qualityInspectionId: String(id) }) } },
