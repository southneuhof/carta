...
const documentationInitial = computed(() => {
  ...
  return Object.fromEntries(documentationNames.flatMap((name) => {
    const documentation = byName.get(name)
    return [[name, documentation?.fileAttachment ?? ''], [`${name}Description`, documentation?.description ?? '']]
  }))
})
const documentationGallery = computed(() => documentationNames.map((name) => {
...
const actionPresentation = {
  ...
  verifyReport: {
    ...
    fields: { resultCode: { label: 'Hasil Inspeksi', form: { renderer: 'radio', source: resultOptions, props: { required: true } } }, description: { label: 'Catatan', form: { renderer: 'textarea' } } },
  },
...
const dialogDisabled = computed(() => submitting.value || (activeAction.value === 'completeReport' && (inspectionPointLoading.value || Boolean(inspectionPointError.value) || !inspectionPointOptions.value.length)))
...
async function loadInspectionPointOptions() {
  ...
  try {
    ...
    inspectionPointOptions.value = template.inspectionPoints
      .filter((point) => (point as { active?: boolean }).active !== false)
      .map((point) => ({ id: point.code, name: point.name }))
  } catch (error) {
...
function actionLabel(action: string) { return action.startsWith('verify:') ? actionPresentation.verifyWorkItem.label : action === 'completeReport' ? actionPresentation.completeReport.label : action === 'verifyReport' ? actionPresentation.verifyReport.label : action === 'submitDocumentations' ? actionPresentation.submitDocumentations.label : action }
function can(action: 'completeReport' | 'submitDocumentations' | 'verifyReport') { return allowedActions.value.includes(actionPresentation[action].serverAction) }
function canItem(row: Record<string, any>, action: string) { return allowedActions.value.includes(action) && Array.isArray(row.allowedActions) && row.allowedActions.includes(action) }
function statusLabel(value: unknown) { return statusLabels[String(value)] ?? String(value ?? '—') }
function statusColor(value: unknown): 'info' | 'warning' | 'success' { return statusColors[String(value)] ?? 'info' }
function resultLabel(value: unknown) { return resultLabels[String(value)] ?? String(value ?? '—') }
function itemResultLabel(value: unknown) { return value === 'waiting' ? 'Menunggu' : resultLabel(value) }
function resultColor(value: unknown): 'success' | 'error' | 'warning' { return resultColors[String(value)] ?? 'warning' }
...
function itemVerifications(row: Record<string, any>) { return Array.isArray(row.verifications) ? row.verifications : [] }
const itemHistory = computed(() => (record.value?.workItems ?? []).flatMap((item: Record<string, any>) => itemVerifications(item).map((event: Record<string, any>) => ({ ...event, itemName: item.workItem?.name ?? item.workItem?.code ?? '—' }))))
const ptsHistory = computed(() => (record.value?.workItems ?? []).flatMap((item: Record<string, any>) => (record.value?.ptsRejections ?? []).filter((event: Record<string, any>) => event.qualityInspectionWorkItemItpId === item.row?.id).map((event: Record<string, any>) => ({ ...event, itemName: item.workItem?.name ?? item.workItem?.code ?? '—', ptsNumber: item.pts?.number ?? '—' }))))
function snapshotTypeLabel(value: unknown) { return itpTypeLabels[String(value)] ?? String(value ?? '—') }
...
function ptsStatusLabel(value: unknown) { return ptsStatusLabels[String(value)] ?? codeLabel(value) }
function ptsStatusColor(value: unknown) { return ptsStatusColors[String(value)] ?? 'info' }
function ptsStepLabel(value: unknown) { return codeLabel(value, ptsStepLabels) }
function ptsRoute(value: Record<string, unknown>) { return { name: 'quality-pts-detail', params: { ptsId: String(value.id) } } as const }
function snapshotsOf(row: Record<string, any>, type: string) { return (Array.isArray(row.snapshots) ? row.snapshots : []).filter((snapshot: Record<string, any>) => snapshot.type === type) }
function criteriaFor(row: Record<string, any>, type: string) { return snapshotsOf(row, type).map((snapshot: Record<string, any>) => snapshot.criteria).filter((criteria: unknown): criteria is string => typeof criteria === 'string' && criteria.trim().length > 0) }
...
<template>
  <div class="flex flex-col gap-2">
    <DetailView :detail="{ fields: detailFields as never, data: record }" title="Detail Laporan" :back-to="{ name: 'quality-quality-inspection' }">
      <template #value:statusCode="{ value }"><Chip variant="tonal" :color="statusColor(value)">{{ statusLabel(value) }}</Chip></template>
      <template #value:resultCode="{ value }"><Chip v-if="value" variant="tonal" :color="resultColor(value)">{{ resultLabel(value) }}</Chip><span v-else>—</span></template>
      <template #controls><QualityInspectionEvidenceExport v-if="record" :record="record" /></template>
    </DetailView>

    <template v-if="record">
      <Card v-if="record.scheduleId" variant="outlined" color="surfaceContainer" class="gap-0 p-0"><header class="border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">Asal Jadwal</h2><p class="mt-1 text-sm text-on-surface-variant">Periode tersimpan pada saat laporan dibuat.</p></header><div class="p-5"><Detail :fields="scheduleFields as never" :data="record" /></div></Card>
      <Card variant="outlined" color="surfaceContainer" class="gap-0 p-0"><header class="flex items-center justify-between gap-2 border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">Prosedur &amp; Penyelesaian</h2><Button v-if="can('completeReport')" type="button" :disabled="submitting" @click="open('completeReport')">{{ actionPresentation.completeReport.label }}</Button></header><div v-if="record.stepCode !== 'report'" class="p-5"><Detail :fields="procedureFields as never" :data="record" /></div><p v-else class="p-5 text-sm text-on-surface-variant">Prosedur dan penyelesaian belum dilengkapi.</p></Card>
      <Card variant="outlined" color="surfaceContainer" class="gap-0 p-0">
        ...
        <Table :data="record.workItems ?? []" :fields="itemFields" :pagination="false" row-key="row.id">
          <template #cell:materialCriteria="{ record: row }"><ol v-if="criteriaFor(row, 'material').length" class="list-decimal ps-5 text-start"><li v-for="criteria in criteriaFor(row, 'material')" :key="criteria">{{ criteria }}</li></ol><span v-else aria-label="Tidak ada kriteria">—</span></template>
          <template #cell:processCriteria="{ record: row }"><ol v-if="criteriaFor(row, 'process').length" class="list-decimal ps-5 text-start"><li v-for="criteria in criteriaFor(row, 'process')" :key="criteria">{{ criteria }}</li></ol><span v-else aria-label="Tidak ada kriteria">—</span></template>
          <template #cell:productCriteria="{ record: row }"><ol v-if="criteriaFor(row, 'product').length" class="list-decimal ps-5 text-start"><li v-for="criteria in criteriaFor(row, 'product')" :key="criteria">{{ criteria }}</li></ol><span v-else aria-label="Tidak ada kriteria">—</span></template>
          <template #cell:pts="{ record: row }"><div v-if="row.pts" class="flex min-w-48 flex-col gap-1"><RouterLink data-testid="pts-link" :to="ptsRoute(row.pts)" class="font-medium text-primary underline-offset-2 hover:underline">{{ row.pts.number }}</RouterLink><div class="flex flex-wrap items-center gap-2"><Chip variant="tonal" :color="ptsStatusColor(row.pts.statusCode)">{{ ptsStatusLabel(row.pts.statusCode) }}</Chip><span class="text-sm text-on-surface-variant">{{ ptsStepLabel(row.pts.stepCode) }}</span></div></div><span v-else>—</span></template>
          ...
          <template #row-actions="{ record: row }"><div v-if="canItem(row, actionPresentation.verifyWorkItem.serverAction)" class="flex gap-1"><Button type="button" variant="tonal" :disabled="submitting" @click.stop="openItem(row, actionPresentation.approve.resultCode)">{{ actionPresentation.approve.label }}</Button><Button type="button" variant="tonal" color="error" :disabled="submitting" @click.stop="openItem(row, actionPresentation.reject.resultCode)">{{ actionPresentation.reject.label }}</Button></div></template>
        </Table>
      </Card>
      <Card variant="outlined" color="surfaceContainer" class="gap-0 p-0"><header class="border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">Data ITP Tersimpan</h2><p class="mt-1 text-sm text-on-surface-variant">Nilai disimpan pada saat laporan dibuat.</p></header><div class="divide-y divide-outline-variant"><section v-for="item in record.workItems ?? []" :key="`snapshot-${item.row.id}`" class="p-5"><h3 class="font-medium">{{ item.workItem?.name }}</h3><p v-if="!item.snapshots?.length" class="mt-2 text-sm text-on-surface-variant">Tidak ada data ITP tersimpan.</p><div v-else class="mt-4 flex flex-col gap-4"><article v-for="snapshot in item.snapshots" :key="snapshot.id" class="rounded-lg border border-outline-variant p-4"><div class="mb-3 flex items-center gap-2"><Chip variant="tonal" color="info">{{ snapshotTypeLabel(snapshot.type) }}</Chip></div><Detail :fields="snapshotFields as never" :data="snapshot" /><div v-if="image(snapshot.imgDocumentation)" class="mt-4 max-w-sm"><p class="mb-2 text-sm font-medium">Foto Dokumentasi</p><ImagePreview :image-u-r-l="image(snapshot.imgDocumentation)" :disable-controls="true" class="aspect-video w-full" /></div><p v-else class="mt-4 text-sm text-on-surface-variant">Foto Dokumentasi: —</p><div v-for="inspector in snapshot.inspectors ?? []" :key="inspector.id" class="mt-4 border-t border-outline-variant pt-4"><Detail :fields="inspectorFields as never" :data="inspector" /><Table v-if="inspector.points?.length" class="mt-3" :data="inspector.points" :fields="snapshotPointFields" :pagination="false"><template #cell:value="{ value }"><Chip :variant="value ? 'tonal' : 'outline'" :color="value ? 'success' : 'error'">{{ value ? 'Ya' : 'Tidak' }}</Chip></template></Table><p v-else class="mt-2 text-sm text-on-surface-variant">Tidak ada Inspection Point.</p></div></article></div></section></div></Card>
      <Card variant="outlined" color="surfaceContainer" class="gap-0 p-0"><header class="border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">Riwayat PTS &amp; Penolakan</h2></header><div class="divide-y divide-outline-variant"><section v-for="item in (record.workItems ?? []).filter((entry: Record<string, any>) => entry.pts)" :key="`pts-${item.row.id}`" class="p-5"><div class="flex flex-wrap items-center gap-2 font-medium"><span>{{ item.workItem?.name }}</span><span aria-hidden="true">·</span><RouterLink data-testid="pts-link" :to="ptsRoute(item.pts)" class="text-primary underline-offset-2 hover:underline">{{ item.pts.number }}</RouterLink><Chip variant="tonal" :color="ptsStatusColor(item.pts.statusCode)">{{ ptsStatusLabel(item.pts.statusCode) }}</Chip><span class="text-sm text-on-surface-variant">{{ ptsStepLabel(item.pts.stepCode) }}</span></div><ul class="mt-2 list-disc ps-5 text-sm"><li v-for="event in (record.ptsRejections ?? []).filter((entry: Record<string, any>) => entry.qualityInspectionWorkItemItpId === item.row.id)" :key="event.id">{{ event.note || 'Ditolak' }} · {{ event.rejectedAt }}</li></ul></section><p v-if="!(record.workItems ?? []).some((entry: Record<string, any>) => entry.pts)" class="p-5 text-sm text-on-surface-variant">Tidak ada PTS terkait.</p></div></Card>
      <template v-if="showDocumentation">
        <QualityInspectionDocumentationForm v-if="showDocumentationForm" :initial="documentationInitial" :submit="submitDocumentations" :submit-label="actionPresentation.submitDocumentations.label" @submitted="refresh" />
        <Card v-else data-testid="documentation-gallery" variant="outlined" color="surfaceContainer" class="gap-0 p-0">
          <header class="border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">Foto Sudut Pengambilan</h2><p class="mt-1 text-sm text-on-surface-variant">Dokumentasi yang tersimpan pada laporan.</p></header>
      ...
      <Card v-if="can('verifyReport')" variant="outlined" color="surfaceContainer" class="gap-0 p-0"><header class="border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">{{ actionPresentation.verifyReport.label }}</h2><p class="mt-1 text-sm text-on-surface-variant">Pilih hasil verifikasi laporan.</p></header><div class="p-5"><Button type="button" :disabled="submitting" @click="open('verifyReport')">{{ actionPresentation.verifyReport.label }}</Button></div></Card>
      <Card variant="outlined" color="surfaceContainer" class="gap-0 p-0">
        <header class="border-b border-outline-variant px-5 py-4"><h2 class="font-semibold">Riwayat Audit</h2><p class="mt-1 text-sm text-on-surface-variant">Semua aktivitas, hasil inspeksi item, verifikasi laporan, dan penolakan PTS.</p></header>
        <div class="divide-y divide-outline-variant">
          <section class="p-5">
            <h3 class="font-medium">Aktivitas</h3>
            <Timeline v-if="record.activity?.length" class="mt-3" :data="record.activity">
              <template #node><span class="size-2 rounded-full bg-primary" /></template>
              <template #header="{ data }"><div class="flex flex-wrap items-center gap-2"><span class="font-medium">{{ data.shortDescription }}</span><span class="text-sm text-on-surface-variant">oleh {{ data.actorName ?? data.actorUserId ?? '—' }}</span></div></template>
              <template #content="{ data }"><div class="text-sm text-on-surface-variant">{{ data.createdAt }}</div><div v-if="data.description" class="mt-1 text-sm">{{ data.description }}</div><div v-else class="mt-1 text-sm text-on-surface-variant">Tidak ada catatan</div></template>
            </Timeline>
          ...
          <section class="p-5">
            ...
            <Table v-if="itemHistory.length" class="mt-3" :data="itemHistory" :fields="itemHistoryFields" :pagination="false" row-key="id">
              <template #cell:result="{ record: event }"><Chip variant="tonal" :color="resultColor(event.resultCode)">{{ itemResultLabel(event.resultCode) }}</Chip></template>
            </Table>
          ...
          <section class="p-5">
            ...
            <Timeline v-if="record.verifications?.length" class="mt-3" :data="record.verifications">
              <template #node><span class="size-2 rounded-full bg-primary" /></template>
              <template #header="{ data }"><div class="flex flex-wrap items-center gap-2"><Chip variant="tonal" :color="resultColor(data.resultCode)">{{ resultLabel(data.resultCode) }}</Chip><span class="text-sm">oleh {{ data.verifierName ?? data.verifierId ?? '—' }}</span></div></template>
              <template #content="{ data }"><div class="text-sm text-on-surface-variant">{{ data.verifiedAt }}</div><div v-if="data.description" class="mt-1 text-sm">{{ data.description }}</div><div v-else class="mt-1 text-sm text-on-surface-variant">Tidak ada catatan</div></template>
            </Timeline>
      ...
      <DialogForm v-if="activeAction && !activeAction.startsWith('verify:') && activeAction !== 'documentation'" :key="`${activeAction}-${dialogKey}`" v-model:open="actionOpen" :title="actionLabel(activeAction)" :description="dialogDescription" :fields="dialogFields as never" :submit="submitAction as never" :disabled="dialogDisabled" />
      <DialogForm v-if="activeAction?.startsWith('verify:')" :key="activeAction" v-model:open="actionOpen" :title="actionPresentation.verifyWorkItem.label" :fields="actionPresentation.verifyWorkItem.fields as never" :submit="(input) => submitItem(rowForVerification(activeAction!), input) as never" :disabled="submitting" />
    </template>
  </div>
</template>
