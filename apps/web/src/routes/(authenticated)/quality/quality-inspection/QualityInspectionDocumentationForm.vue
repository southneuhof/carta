...
const props = defineProps<{ initial?: Record<string, unknown>; submit: (input: SubmitQualityInspectionDocumentations) => Promise<unknown> }>()
...
async function submitForm(value: Record<string, unknown>) {
  if (!names.every((name) => path(value[name]))) throw new Error('All four documentation files are required.')
