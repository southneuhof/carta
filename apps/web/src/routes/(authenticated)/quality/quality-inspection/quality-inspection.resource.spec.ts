...
describe('Inspection/Test resource', () => {
  it('keeps legacy labels, field order, owner sources, and standard routes', () => {
    ...
    expect(root.behavior?.props?.({ draft: { projectId: 'project-1', qualityWorkCategoryId: 'category-1' } } as never)).toEqual({
      searchParameters: { projectId: 'project-1', categoryId: 'category-1', rootOnly: true, active: true },
    })
  ...
  it('uses legacy status labels and validates selected rows', () => {
    ...
    expect(table.map((field) => field.label)).toEqual([
      'Nomor Inspection/Test',
      'Proyek',
      'Target Pelaksanaan',
      'Jenis Pekerjaan',
      'Dilaporkan Oleh',
    ...
    expect(
      qualityInspection.create().schema?.validate({ divisionId: 'd1', projectId: 'p1', targetDate: '2026-08-20', qualityWorkCategoryId: 'c1', workItemCategoryId: 'w1', selectedRows: [] }).success
    ).toBe(false)
    expect(
      qualityInspection
        .create()
        .schema?.validate({
          ...
          qualityWorkCategoryId: 'c1',
          workItemCategoryId: 'w1',
          selectedRows: [{ workItemId: 'leaf', volume: 1, itpTypeCodes: ['material'] }],
        }).success
    ).toBe(true)
  })
