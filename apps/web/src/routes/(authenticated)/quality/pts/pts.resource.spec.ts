...
describe('manual PTS resource', () => {
  ...
  it('uses owner resources and approved searchParameters', () => {
    ...
    expect(ptsActionFields.rootCauseIds.form.props).toMatchObject({ multi: true, required: true })
    expect(ptsActionFields.criteriaCode.form.props).toMatchObject({ required: true })
    expect(ptsActionFields.imgBefore.form.props).toMatchObject({ required: true })
    expect(ptsActionFields.location.form.props).toMatchObject({ required: true })
  ...
  it('exposes standard routes and plain workflow actions', () => {
    ...
    expect(pts.actions.completeQiReport.run).toEqual(expect.any(Function))
  })
})
