...
const fields = defineFields(ptsSchema, {
  ...
  divisionId: {
    ...
    form: {
      ...
      behavior: { props: () => ({ searchParameters: { permission: 'create-qhsse-pts', active: true } }) },
    },
  },
  projectId: {
    label: 'Project',
  ...
  rootCauseIds: {
    ...
    read: (record) =>
      Array.isArray(record.rootCauses)
        ? record.rootCauses
            .map((cause) => (cause as { name?: string }).name)
            .filter(Boolean)
            .join(', ')
        : '',
    form: { renderer: 'lookup', source: rootCauses, props: { pick: 'id', view: 'name', multi: true, required: true }, behavior: { props: () => ({ searchParameters: { active: true } }) } },
    write: writeRootCauseIds,
  },
...
export const ptsActionFields = {
  ...
  somUserId: {
    ...
    form: {
      ...
      source: users,
      props: { pick: 'id', view: 'name', required: true },
      behavior: { props: ({ draft }) => ({ searchParameters: { projectId: draft.projectId, statusCode: 'active' } }) },
    },
  },
