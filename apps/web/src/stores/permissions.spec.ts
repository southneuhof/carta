...
describe('permissions store', () => {
  ...
  it('builds only from an explicit permission array', () => {
    ...
    expect(store.can('stale-permission')).toBe(true)

    store.build([])

    expect(store.can('stale-permission')).toBe(false)
  })

  it('does not grant permissions without an explicit code', () => {
    expect(permissions().can('view-unlisted-route')).toBe(false)
  })

  it('updates computed consumers when permissions are built or cleared', () => {
    const store = permissions()
    const canViewUsers = computed(() => store.can('view-users'))
